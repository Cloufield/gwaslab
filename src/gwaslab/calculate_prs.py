...
def _calculate_prs(sumstats, 
    ...
    #matching_alleles
        ...
        bfile_prefix, plink_log, ref_bim = _process_vcf_and_bfile(  chrlist=chrlist,
                                                                    bfile=bfile, 
                                                                    vcf=vcf, 
                                                                    plink_log=plink_log,
                                                                    n_cores=n_cores, 
...
def _combine_all_chr_prs(score_file_path_list):
    ...
    for index, score_file_path in enumerate(score_file_path_list):
        ...
        gc.collect()
    combined_results_summary = combined_results.groupby(["#FID","IID"])[["ALLELE_CT","DENOM","NAMED_ALLELE_DOSAGE_SUM","SCORE1_SUM"]].sum()
...
def _run_calculate_prs(study, chrom , model_path, bfile_prefix, n_cores, out, plink_log, log, memory, mode=None):
    
    log.write(" -Start to calculate PRS for Chr {}...".format(chrom))
    _checking_plink_version(v=2, log=log)
    
    if "@" in bfile_prefix:
        bfile_to_use = bfile_prefix.replace("@",str(chrom))
    else:
    ...
    if os.path.exists(bfile_to_use+".bed"):
        ...
        plink2 \
            ...
            --threads {} {}\
            --out {}
        """.format(bfile_to_use, model_path ,  mode if mode is not None else "", chrom, n_cores, memory_flag if memory is not None else "", output_prefix)
...
def _match_snpid_with_bim(chrom, chr_sumstats, bfile_prefix, id_to_use=None, log=Log()):
