...
def fill_p(sumstats,log,df=None,only_sig=False,overwrite=False,verbose=True,filled_count=0):
        # MLOG10P -> P
...
def fill_iteratively(sumstats,to_fill,log,only_sig,df,extreme,verbose):
    ...
    # beta to or ####################################################################################################     
        if "OR" in to_fill:
            fill_or(sumstats,log,verbose=verbose,filled_count=filled_count)
            
    # or to beta #################################################################################################### 
        if "BETA" in to_fill:
            fill_beta(sumstats,log,verbose=verbose,filled_count=filled_count)
        if "SE" in to_fill:
            fill_se(sumstats,log,verbose=verbose,filled_count=filled_count)
    # z/chi2 to p ##################################################################################################
        if "P" in to_fill:
            fill_p(sumstats,log,only_sig=only_sig,df=df,verbose=verbose,filled_count=filled_count)
    # beta/se to z ##################################################################################################            
        if "Z" in to_fill:   
            fill_z(sumstats,log,verbose=verbose,filled_count=filled_count)
    # z/p to chisq ##################################################################################################             
        if "CHISQ" in to_fill:
            fill_chisq(sumstats,log,verbose=verbose,filled_count=filled_count)
    # EAF to MAF ##################################################################################################   
        if "MAF" in to_fill:
            fill_maf(sumstats,log,verbose=verbose,filled_count=filled_count)
    # p to -log10(P)  ###############################################################################################
        if "MLOG10P" in to_fill:
            if extreme==True:
                fill_extreme_mlog10p(sumstats,log,verbose=verbose,filled_count=filled_count)
                filled_count +=1
            elif "P" not in sumstats.columns:
                fill_p(sumstats,log,verbose=verbose)
                fill_mlog10p(sumstats,log,verbose=verbose,filled_count=filled_count)
                sumstats = sumstats.drop(labels=["P"],axis=1)
            else:
                fill_mlog10p(sumstats,log,verbose=verbose)
        if filled_count == len(to_fill):
            break